...
class NBAScene(MovingCameraScene):
    ...
    def add_games_count(self, count=3):
        ...
        total = Text("Total", font="DDTW00-CondensedBoldItalic", color="#cd8585").scale(.35)
...
class Test(NBAScene):
    def construct(self):
        ...
        self.play(ShowCreation(self.main_line))
        self.play(ShowCreation(self.main_dividers))
        self.play(ShowCreation(self.team_dividers))
        self.play(ShowCreation(self.categories))
        self.play(ShowCreation(self.box_divider))
        a = NumberAnimation(0, 0)
        self.add(a)
        self.play(a.increment(5))
        self.wait()
        self.play(a.increment(20))
